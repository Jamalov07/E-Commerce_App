...
let productInCart = document.querySelector(".offcanvas-body");
...
function toCart() {
  cartNumber.textContent = productCartList.length;
  productInCart.innerHTML = "";
  productCartList.forEach((pro) => {
    productInCart.append(addToCart(pro));
  });
  totalSum();
}
    ...
    <div class="col-md-8">
      <div class="card-body">
        ...
        >
          ...
          <button class="btn btn-danger" id=${product.id}>
...
const addNewCard = (product) => {
  ...
  <div class="card-body">
    ...
    <button class="btn-add btn btn-primary" id="${product.id}">Add Cart</button>
  </div>
  </div>`
  );
  return element;
...
products.forEach((pro) => {
  productWrapper.append(addNewCard(pro));
});
...
productWrapper.addEventListener("click", (e) => {
  ...
  if (id) {
    products.forEach((pro) => {
      if (pro.id === id) {
        let idList = [];
        productCartList.forEach((pr) => {
          idList.push(pr.id);
        });
        if (idList.includes(id)) {
          alert("Bu buyurtma allaqachon qo'shilgan");
        } else {
          productCartList.push(pro);
          toCart();
        }
      }
    });
...
productInCart.addEventListener("click", (e) => {
  ...
  if (id) {
    productCartList = productCartList.filter((pr) => pr.id != id);
    toCart();
  }
});
...
inputSearch.addEventListener("input", () => {
  ...
  products.forEach((pr) => {
    if (
      ...
      pr.name.slice(0, text.length).toLocaleLowerCase().trim()
    ) {
      list.push(pr);
    }
  });
